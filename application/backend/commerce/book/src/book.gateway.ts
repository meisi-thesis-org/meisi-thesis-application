...
import { CreateBookSchema, FindBookByUuidSchema, FindBooksByDossierUuidSchema, UpdateBookByUuidSchema } from './structs/book.schema';
...
export class BookGateway extends Gateway<Router> {
  ...
  public override subscribe (): Router {
    ...
    this.router.get('/', SchemaValidator(FindBooksByDossierUuidSchema), async (request: Request, response: Response) => await this.controller.findBooksByDossierUuid(request, response))
