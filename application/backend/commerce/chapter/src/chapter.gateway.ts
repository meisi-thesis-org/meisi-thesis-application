...
import { CreateChapterSchema, FindChapterByUuidSchema, FindChaptersByBookUuidSchema, UpdateChapterByUuidSchema } from './structs/chapter.schema';
...
export class ChapterGateway extends Gateway<Router> {
  ...
  public override subscribe (): Router {
    ...
    this.router.get('/', SchemaValidator(FindChaptersByBookUuidSchema), async (request: Request, response: Response) => await this.controller.findChaptersByBookUuid(request, response))
